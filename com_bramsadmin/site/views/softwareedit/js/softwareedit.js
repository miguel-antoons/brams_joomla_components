/* eslint-disable no-global-assign */
// * cf. ../../_js/edit.js
// eslint-disable-next-line no-unused-vars
/* global $, elementId, codes, log, apiFailMessg, newElement, updateElement, getCodes */
const currentView = 'softwareEdit';
const redirectView = 'softwares';

/**
 * Function checks if all the required inputs have values in them. It doesn't
 * check the values itself, just if there is something.
 *
 * @param softwareCode  {string}    softwareCode input value
 * @param oldIsValid    {boolean}   flag that determines if values are valid or not
 * @returns             {(*|string)[]|(boolean|string)[]}
 *                                  Returns an array with 2 values :
 *                                      0: new isValid flag
 *                                      1: error message if an input is empty
 */
function verifyRequired(softwareCode, oldIsValid) {
    // if one of the required inputs are empty
    if (!softwareCode) {
        // add an exclamation circle to the required inputs
        const requiredInputs = document.getElementsByClassName('required');
        Array.from(requiredInputs).forEach(
            (input) => {
                input.innerHTML
                    += '<i class="fa fa-exclamation-circle orange right" aria-hidden="true"></i>';
            },
        );

        // add an error text
        return [
            false,
            `<li>
                <i class="fa fa-exclamation-circle orange" aria-hidden="true"></i>
                Please fill all required inputs before submitting the form. 
                Required inputs are Software Code.
            </li>`,
        ];
    }

    return [oldIsValid, ''];
}

/**
 * Function verifies if the entered software code doesn't exist already.
 * This is done so that each software code is unique.
 *
 * @param softwareCode  {string}    softwareCode input value
 * @param oldIsValid    {boolean}   flag that determines if values are valid or not
 * @returns             {(*|string)[]|(boolean|string)[]}
 *                                  Returns an array with 2 values :
 *                                      0: new isValid flag
 *                                      1: error message if the software code already exists
 */
function verifyCode(softwareCode, oldIsValid) {
    const pattern = /^[a-z\d\-_]+$/i;
    // if the antenna code already exists
    if (codes.includes(softwareCode)) {
        document.getElementById('code').innerHTML += ''
            + '<i class="fa fa-exclamation-circle red right" aria-hidden="true"></i>';

        // set the valid flag to false and return an error message
        return [
            false,
            `<li>
                <i class="fa fa-exclamation-circle red" aria-hidden="true"></i>
                Entered software code is already taken. Please enter a free software code.
            </li>`,
        ];
    }

    // test if any forbidden characters are in the code
    if (!pattern.test(softwareCode)) {
        document.getElementById('code').innerHTML += ''
            + '<i class="fa fa-exclamation-circle red right" aria-hidden="true"></i>';

        // set the valid flag to false and return an error message
        return [
            false,
            `<li>
                <i class="fa fa-exclamation-circle red" aria-hidden="true"></i>
                Entered software code contains forbidden characters. Be sure to only use dash, 
                underscore and alphanumeric characters.
            </li>`,
        ];
    }

    return [oldIsValid, ''];
}

/**
 * Function verifies if the entered values are valid upon api call. If all the
 * values are valid, the function returns true. If not it returns false.
 *
 * @param softwareCode  {string}    softwareCode input value
 * @returns             {boolean}   true if the inputs are valid, else returns false
 */
function verifyValues(softwareCode) {
    // remove all the icons inside the labels
    const icons = document.querySelectorAll('label .fa');
    let verificationResult;
    icons.forEach((icon) => icon.remove());

    let isValid = true;
    let HTMLError = '<h4>Found Problems</h4><ul>';

    // check if all required inputs are filled
    verificationResult = verifyRequired(softwareCode, isValid);
    [isValid] = verificationResult;
    HTMLError += verificationResult[1];

    // check if antenna code is valid
    verificationResult = verifyCode(softwareCode, isValid);
    [isValid] = verificationResult;

    // display the errors on the page
    document.getElementById('error').innerHTML = `${HTMLError}${verificationResult[1]}</ul>`;

    return isValid;
}

/**
 * Function calls an api to create a new software. It sends all the information
 * about the new software to back-end. For the api to be called, the input values
 * have to be valid.
 *
 * @param formInputs    {HTMLDivElement.children}    Div element containing the inputs
 */
function newSoftware(formInputs) {
    const sofCode = formInputs.softwareCode.value;
    const sofName = formInputs.softwareName.value;
    const sofVersion = formInputs.softwareVersion.value;
    const sofComments = formInputs.softwareComments.value;

    // verify if the entered values are valid
    if (verifyValues(sofCode)) {
        const data = {
            new_software: {
                code: sofCode,
                name: sofName,
                version: sofVersion,
                comments: sofComments,
            },
        };
        newElement(data, currentView, redirectView);
    }
}

/**
 * Function calls an api to update a software entry. It sends all the information
 * about the updated software to back-end. For the api to be called, the input values
 * have to be valid.
 *
 * @param formInputs    {HTMLDivElement.children}    Div element containing the inputs
 */
function updateSoftware(formInputs) {
    const sofCode = formInputs.softwareCode.value;
    const sofName = formInputs.softwareName.value;
    const sofVersion = formInputs.softwareVersion.value;
    const sofComments = formInputs.softwareComments.value;

    // verify if all the entered values are valid
    if (verifyValues(sofCode)) {
        const data = {
            modified_software: {
                id: elementId,
                code: sofCode,
                name: sofName,
                version: sofVersion,
                comments: sofComments,
            },
        };
        updateElement(data, currentView, redirectView);
    }
}

// function decides which api to call (update or create)
// eslint-disable-next-line no-unused-vars
function formProcess(form) {
    if (elementId) {
        return updateSoftware(form);
    }
    return newSoftware(form);
}

/**
 * Function gets all the information about a specific software through
 * an api to the sites back-end. The software to get information from
 * is specified in the pages url.
 * It then prepares the page to update/create a software.
 */
function getSoftwareInfo() {
    // get the id from url
    const paramString = window.location.search.split('?')[1];
    const queryString = new URLSearchParams(paramString);
    elementId = Number(queryString.get('id'));
    // get all the software codes
    getCodes(currentView);

    // if a software has to be updated
    if (elementId) {
        const token = $('#token').attr('name');

        $.ajax({
            type: 'GET',
            url: `
                index.php?
                option=com_bramsadmin
                &task=getOne
                &view=${currentView}
                &format=json
                &id=${elementId}
                &${token}=1
            `,
            success: (response) => {
                document.getElementById('softwareCode').value = response.data.code;
                document.getElementById('softwareName').value = response.data.name;
                document.getElementById('softwareVersion').value = response.data.version;
                document.getElementById('softwareComments').value = response.data.comments;
                document.getElementById('title').innerHTML = `
                    Update Software ${response.data.code}`;
            },
            error: (response) => {
                // on fail, show an error message
                document.getElementById('error').innerHTML = apiFailMessg;
                // store the server response in the log variable
                log = response;
            },
        });
    }
}

// set onload function
window.onload = getSoftwareInfo;
