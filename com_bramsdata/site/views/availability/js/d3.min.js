...
(function (global, factory) {
  ...
  function freedmanDiaconis(values, min, max) {
	  ...
	  return Math.ceil((max - min) / (2 * (threshold(values, 0.75) - threshold(values, 0.25)) * values.length ** (-1 / 3)));
  }

  function scott(values, min, max) {
	  return Math.ceil((max - min) / (3.5 * deviation(values) * values.length ** (-1 / 3)));
  }
  ...
  const keyPrefix = '$'; // Protect against keys like â€œ__proto__â€.
  ...
  function dragDisable(view) {
	  const root = view.document.documentElement;
		  const selection$$1 = select(view).on('dragstart.drag', noevent, true);
  ...
  function yesdrag(view, noclick) {
	  const root = view.document.documentElement;
		  const selection$$1 = select(view).on('dragstart.drag', null);
	  if (noclick) {
      selection$$1.on('click.drag', noevent, true);
      setTimeout(() => { selection$$1.on('click.drag', null); }, 0);
	  }
	  if ('onselectstart' in root) {
  ...
  function drag() {
      ...
      const touches$$1 = exports.event.changedTouches;
      const c = container.apply(this, arguments);
      const n = touches$$1.length; let i; let
        gesture;
      ...
      let p = point$$1(container, id); let s; let dx; let dy;
  ...
  function exponential(a, b, y) {
	  return a **= y, b = b ** y - a, y = 1 / y, function (t) {
      return (a + t * b) ** y;
	  };
  }
  ...
  const interpolateRgb = (function rgbGamma(y) {
	  const color$$1 = gamma(y);

	  function rgb$$1(start, end) {
      const r = color$$1((start = rgb(start)).r, (end = rgb(end)).r);
      const g = color$$1(start.g, end.g);
      const b = color$$1(start.b, end.b);
  ...
  function hsl$1(hue$$1) {
      ...
      const h = hue$$1((start = hsl(start)).h, (end = hsl(end)).h);
  ...
  function hcl$1(hue$$1) {
	  return function (start, end) {
      const h = hue$$1((start = hcl(start)).h, (end = hcl(end)).h);
  ...
  function cubehelix$1(hue$$1) {
      ...
      function cubehelix$$1(start, end) {
		  const h = hue$$1((start = cubehelix(start)).h, (end = cubehelix(end)).h);
			  const s = nogamma(start.s, end.s);
			  const l = nogamma(start.l, end.l);
			  const opacity = nogamma(start.opacity, end.opacity);
		  return function (t) {
  ...
  function tweenRemove(id, name) {
      ...
      tween1;
	  return function () {
      const schedule$$1 = set$1(this, id);
      const { tween } = schedule$$1;
  ...
  function styleMaybeRemove(id, name) {
      ...
      const schedule$$1 = set$1(this, id);
      const { on } = schedule$$1;
      const listener = schedule$$1.value[key] == null ? remove || (remove = styleRemove$1(name)) : undefined;
  ...
  function brush$1(dim) {
	  let extent = defaultExtent;
		  ...
		  const listeners = dispatch(brush, 'start', 'brush', 'end');
		  let handleSize = 6;
		  let touchending;
      ...
      if (group.selection) {
		  group
            ...
            const selection1 = dim.input(typeof selection$$1 === 'function' ? selection$$1.apply(this, arguments) : selection$$1, state.extent);
		  ...
		  group
            ...
            const state = that.__brush;
            const selection1 = dim.input(typeof selection$$1 === 'function' ? selection$$1.apply(that, args) : selection$$1, state.extent);
      ...
      const selection$$1 = local$1(this).selection;
      ...
      if (selection$$1) {
		  ...
		  group.selectAll('.handle')
			  .style('display', null)
			  .attr('x', (d) => (d.type[d.type.length - 1] === 'e' ? selection$$1[1][0] - handleSize / 2 : selection$$1[0][0] - handleSize / 2))
			  .attr('y', (d) => (d.type[0] === 's' ? selection$$1[1][1] - handleSize / 2 : selection$$1[0][1] - handleSize / 2))
			  .attr('width', (d) => (d.type === 'n' || d.type === 's' ? selection$$1[1][0] - selection$$1[0][0] + handleSize : handleSize))
			  .attr('height', (d) => (d.type === 'e' || d.type === 'w' ? selection$$1[1][1] - selection$$1[0][1] + handleSize : handleSize));
      } else {
      ...
      let mode = (exports.event.metaKey ? type = 'overlay' : type) === 'selection' ? MODE_DRAG : (exports.event.altKey ? MODE_CENTER : MODE_HANDLE);
      let signX = dim === Y ? null : signsX[type];
      let signY = dim === X ? null : signsY[type];
      const state = local$1(that);
      const { extent } = state;
      let selection$$1 = state.selection;
      ...
      let dy;
      let moving;
      let shifting = signX && signY && exports.event.shiftKey;
      let lockX;
      let lockY;
      const point0 = mouse(that);
      let point$$1 = point0;
      const emit = emitter(that, arguments).beforestart();
  ...
  function contours() {
	  let dx = 1;
		  let dy = 1;
		  let threshold$$1 = thresholdSturges;
      ...
      let x; let y; let t0; let t1; let t2; let
        t3;
  ...
  function parser(type) {
      ...
      return text(input, init).then((text$$1) => (new DOMParser()).parseFromString(text$$1, type));
	  };
  }
  ...
  function collide(radius) {
      ...
      function apply(quad, x0, y0, x1, y1) {
		  const { data } = quad; let rj = quad.r; let
          r = ri + rj;
		  if (data) {
  ...
  function simulation(nodes) {
	  let simulation;
		  let alpha = 1;
		  let alphaMin = 0.001;
		  let alphaDecay = 1 - alphaMin ** (1 / 300);
		  let alphaTarget = 0;
  ...
  function formatLocale(locale) {
	  const group = locale.grouping && locale.thousands ? formatGroup(locale.grouping, locale.thousands) : identity$3;
		  const { currency } = locale;
		  const { decimal } = locale;
		  const numerals = locale.numerals ? formatNumerals(locale.numerals) : identity$3;
      ...
      const prefix = symbol === '$' ? currency[0] : symbol === '#' && /[boxX]/.test(type) ? `0${type.toLowerCase()}` : '';
      const suffix = symbol === '$' ? currency[1] : /[%p]/.test(type) ? percent : '';
  ...
  function linePoint(lambda, phi) {
      ...
      const sign$$1 = delta > 0 ? 1 : -1;
      let lambdai = inflection[0] * degrees$1 * sign$$1;
      let phii;
  ...
  function containsLine(coordinates, point) {
	  const ab = distance(coordinates[0], coordinates[1]);
		  const ao = distance(coordinates[0], point);
		  const ob = distance(point, coordinates[1]);
  ...
  function projectionMutator(projectAt) {
      ...
      const transform$$1 = (alpha ? scaleTranslateRotate : scaleTranslate)(k, x - center[0], y - center[1], alpha);
  ...
  function conicConformalRaw(y0, y1) {
      ...
      return [atan2(x, abs(fy)) / n * sign(fy), 2 * atan((f / r) ** (1 / n)) - halfPi$2];
	  };
  ...
  function identity$5() {
	  let k = 1; let tx = 0; let ty = 0; let sx = 1; let sy = 1; let transform$$1 = identity$4; // scale, translate and reflect
  ...
  const keyPrefix$1 = '$'; // Protect against keys like â€œ__proto__â€.
  ...
  function transformer$1() {
	  let domain = unit;
		  let range = unit;
		  let interpolate$$1 = interpolateValue;
		  let transform;
		  let untransform;
		  let unknown;
  ...
  function voronoi() {
      ...
      return new Diagram(data.map((d, i) => {
		  const s = [Math.round(x$$1(d, i, data) / epsilon$4) * epsilon$4, Math.round(y$$1(d, i, data) / epsilon$4) * epsilon$4];
		  s.index = i;
		  s.data = d;
		  return s;
      }), extent);
  ...
  function zoom() {
      ...
      const selection$$1 = collection.selection ? collection.selection() : collection;
      ...
      const t = this.__zoom;
      const k = Math.max(scaleExtent[0], Math.min(scaleExtent[1], t.k * 2 ** wheelDelta.apply(this, arguments)));
      const p = mouse(this);
      ...
      const touches$$1 = exports.event.changedTouches;
      let started;
      const n = touches$$1.length; let i; let t; let
        p;
      ...
      const g = gesture(this, arguments);
      const touches$$1 = exports.event.changedTouches;
      const n = touches$$1.length; let i; let t; let p; let
        l;
