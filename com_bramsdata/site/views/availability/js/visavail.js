(function (global, factory) {
  ...
  function visavailChart(custom_options, dataset) {
    ...
    function convertMomentToStrftime(momentFormat) {
      const replacements = {
        ddd: 'a', dddd: 'A', MMM: 'b', MMMM: 'B', lll: 'c', DD: 'd', D: 'e', 'YYYY-MM-DD': 'F', HH: 'H', H: 'H', hh: 'I', h: 'I', DDDD: 'j', DDD: '-j', MM: 'm', M: '-m', mm: 'M', m: '-M', A: 'p', a: 'P', ss: 'S', s: '-S', E: 'u', d: 'w', WW: 'W', ll: 'x', LTS: 'X', YY: 'y', YYYY: 'Y', ZZ: 'z', z: 'Z', SSS: 'L', '%': '%',
      };
      const tokens = momentFormat.split(/( |\/|:|,|\]|\[|\.)/);

      const strftime = tokens.map((token) => {
        // Replace strftime tokens with moment formats
        if (token[0] == ':' || token[0] == '/' || token[0] == ' ' || token[0] == ',') { return token; }
      ...
      selection.each(function drawGraph(dataset) {
        ...
        if (options.display_date_range && (options.display_date_range[0] || options.display_date_range[1])) {
          if (options.display_date_range[0]) {
            ...
            } else if (options.date_in_utc) { startDate = moment.utc(options.display_date_range[0]).toDate(); } else { startDate = options.display_date_range[0]; }
          }
          if (options.display_date_range[1]) {
            ...
            } else if (options.date_in_utc) { endDate = moment.utc(options.display_date_range[1]).toDate(); } else { endDate = options.display_date_range[1]; }
          }
        }
        ...
        if (options.y_percentage.enabled && !options.y_percentage.custom_percentage) {
          dataset.forEach((series, seriesI) => {
            ...
            if (!options.custom_categories) {
              series.disp_data.forEach((disp) => {
                if (disp[1]) dataset[seriesI].timeup_ms += disp[2].getTime() - disp[0].getTime();
                else dataset[seriesI].timedown_ms += disp[2].getTime() - disp[0].getTime();
              });
            } else {
              series.disp_data.forEach((disp) => {
                if (disp[1] === series.category_percentage) dataset[seriesI].timeup_ms += disp[2].getTime() - disp[0].getTime();
                else dataset[seriesI].timedown_ms += disp[2].getTime() - disp[0].getTime();
              });
            }
          });
        ...
        if (options.show_y_title) {
          ...
          const y_axis_title = svg.select('#g_axis_text').append('g').attr('id', 'yAxis');
          ...
          y_axis_title.selectAll('g').append('text')
            ...
            .attr('y', (d, i) => ((options.line_spacing + options.graph.height) * i) + options.line_spacing + (options.graph.height) / 2)
            .attr('dy', '0.5ex')
            .text((d) => d.measure || d.measure_html,
            ...
            .attr('class', (d) => {
              ...
              if (options.y_percentage.enabled && !options.y_percentage.custom_percentage) {
                if (d.timedown_ms == 0) { returnCSSClass += ` ${options.y_title_total_availability_class}`; } else
                if (d.timeup_ms == 0) { returnCSSClass += ` ${options.y_title_total_unavailability_class}`; } else { returnCSSClass += ` ${options.y_title_some_unavailability_class}`; }
              }
          ...
          y_axis_title.selectAll('g')
            .append('circle')
            .attr('class', (d) => `ytitle_icon_background ${(d.icon || '').background_class || ''}`)
            .attr('cx', (d) => {
              if (d.icon && d.icon.width) { return options.padding.left - d.icon.width / 2 - ((d.icon.padding || 0).right || 0) - (d.icon.padding_right || 0); }
              return 0;
            })
            .attr('cy', (d, i) => {
              if (d.icon && d.icon.height) { return ((options.line_spacing + options.graph.height) * i) + options.line_spacing + (options.graph.height) / 2; }
              return 0;
            })
            .attr('r', (d) => {
              if (d.icon && d.icon.width) { return d.icon.width / 2; }
              return 0;
            });

          y_axis_title.selectAll('g')
            .append('image')
            .attr('class', (d) => `ytitle_icon_image ${(d.icon || '').image_class || ''}`)
            .attr('xlink:href', (d) => {
              if (d.icon && d.icon.url) { return d.icon.url; }
              return '';
            })
            .attr('x', (d) => {
              if (d.icon && d.icon.width) { return options.padding.left - d.icon.width - ((d.icon.padding || 0).right || 0) - (d.icon.padding_right || 0); }
              return 0;
            })
            .attr('y', (d, i) => {
              if (d.icon && d.icon.height) { return ((options.line_spacing + options.graph.height) * i) - d.icon.height / 2 + options.line_spacing + (options.graph.height) / 2; }
              return 0;
            })
            .attr('width', (d) => {
              if (d.icon && d.icon.width) { return d.icon.width; }
              return 0;
            })
            .attr('height', (d) => {
              if (d.icon && d.icon.height) { return d.icon.height; }
              return 0;
            });
          ...
          if (options.y_title_tooltip.enabled) {
            ...
            y_axis_title.selectAll('g').append('title')
              .text((d) => d.measure_description || d.measure);
          }
        ...
        if (options.y_percentage.enabled) {
          // create y axis labels
          const y_percentage_title = svg.select('#g_axis_text').append('g').attr('id', 'yPercentage');
          ...
          y_percentage_title.selectAll('g').append('text')
            ...
            .text((d) => {
              if (!options.custom_categories && options.y_percentage.unavailability_percentage) { return options.y_percentage.percentageFormat(d.timedown_ms / (d.timedown_ms + d.timeup_ms)); }
              if (options.y_percentage.custom_percentage) { return (d.percentage || '').measure || ''; }
              return options.y_percentage.percentageFormat(d.timeup_ms / (d.timeup_ms + d.timedown_ms));
            })
            .each(wrap)
            .attr('class', (d) => {
              let returnCSSClass = 'ypercentage';
              if (!options.y_percentage.custom_percentage) {
                if (d.timedown_ms == 0) { returnCSSClass += ` ${options.y_percentage.total_availability_class}`; } else
                if (d.timeup_ms == 0) { returnCSSClass += ` ${options.y_percentage.total_unavailability_class}`; } else { returnCSSClass += ` ${options.y_percentage.some_unavailability_class}`; }
              } else if (options.y_percentage.custom_percentage) { returnCSSClass += ` ${(d.percentage || '').class}` || ''; }
        ...
        svg.select('#g_axis').append('g').attr('id', 'hGrid').selectAll('line.horz_grid')
          .data(dataset)
          .enter()
          .append('line')
          .attr('x1', 0)
          .attr('x2', width)
          .attr('y1', (d, i) => ((options.line_spacing + options.graph.height) * i) + options.line_spacing + options.graph.height / 2)
          .attr('y2', (d, i) => ((options.line_spacing + options.graph.height) * i) + options.line_spacing + options.graph.height / 2)
        ...
        const g = svg.select('#g_data').selectAll('.g_data')
          ...
          .enter()
          .append('g')
          .attr('transform', (d, i) => `translate(0,${(options.line_spacing + options.graph.height) * i})`)
          .attr('cursor', 'pointer')
          .attr('class', 'dataset');
        ...
        g.selectAll('rect')
          ...
          .enter()
          .append('rect')
          .attr('x', (d) => xForPoint(d, options.graph.width, options.xScale, 0))
          .attr('width', (d) => widthForPoint(d, options.graph.width, options.xScale, 0))
          .attr('y', options.line_spacing)
          .attr('height', options.graph.height)
          .attr('transform', (d) => transformForTypeOfGraph(d, options.xScale, options.graph.height, options.line_spacing, 0))
          .attr('rx', (d) => roundedRect())
          .attr('ry', (d) => roundedRect())
          .attr('class', (d) => {
            if (options.custom_categories) {
              const series = dataset.filter(
                (series) => series.disp_data.indexOf(d) >= 0,
              )[0];
        ...
        function redrawTooltipWhenOver(obj, dataset, pageX, pageY, d, i) {
          if (options.tooltip.hover_zoom.enabled) {
            d3.select(obj).transition()
              ...
              .attr('transform', (d) => transformForTypeOfGraph(d, options.xScale, options.graph.height, options.line_spacing, options.line_spacing * options.tooltip.hover_zoom.ratio));
          }
          const matrix = obj.getCTM().translate(obj.getAttribute('x'), obj.getAttribute('y'));
          div.transition()
            .duration(options.tooltip.duration)
          ...
          div.html(() => {
            ...
            if (options.custom_categories) {
              ...
              if (series && series.categories[d[1]].tooltip_html) { output = series.categories[d[1]].tooltip_html; } else { output = `&nbsp;${d[1]}&nbsp;`; }
            } else if (d[1] === 1) {
              // checkmark icon
        ...
        function emphasize(scale) {
          ...
          if (options.emphasize_year_ticks
						&& !(isYearTick.every((d) => d === true))
						&& isMonthTick.every((d) => d === true)) {
            d3.selectAll(`#${options.id_div_graph} g.tick`).each(function (_d, i) {
              if (isYearTick[i]) {
                d3.select(this)
        ...
        if (options.sub_chart.enabled) {
          ...
          if (options.sub_chart.graph.enabled) {
            svg.selectAll('#g_sub_data').selectAll('.g_sub_data')
              ...
              .enter()
              .append('g')
              .attr('transform', (d, i) => `translate( 0, ${((options.sub_chart.graph.line_spacing + options.sub_chart.graph.height) * i) + options.sub_chart.margin.top + height})`)

              .selectAll('rect')
              .data((d) => d.disp_data)
              .enter()
              .append('rect')
              .attr('x', (d) => xForPoint(d, options.sub_chart.graph.width, xScale2, 0))
              .attr('width', (d) => widthForPoint(d, options.sub_chart.graph.width, xScale2, 0))
              .attr('y', options.sub_chart.graph.line_spacing)
              .attr('height', options.sub_chart.graph.height)
              .attr('transform', (d) => transformForTypeOfGraph(d, xScale2, options.sub_chart.graph.height, options.sub_chart.graph.line_spacing, 0))
              .attr('rx', (d) => roundedRect())
              .attr('ry', (d) => roundedRect())
              .attr('class', (d) => {
                if (options.custom_categories) {
        ...
        } else if (options.sub_chart.enabled) { svg.select('#g_brush').call(brush.move, xScale.range()); }
        ...
        function widthForPoint(d, graph_width, xScale, ratio) {
          ...
          if (x_scale_d2 > width) { return width - x_scale_d0 < 0 ? 0 : (width - x_scale_d0 + ratio); }
