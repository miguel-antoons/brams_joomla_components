...
$(document).ready(function(){
        $("img.bramsImage").load(function(){
        });
        startDate=getStartDate();
        endDate=getEndDate();
    $( ".tDate" ).datepicker({dateFormat: "yy-mm-dd", changeYear:true, changeMonth:true, minDate:startDate, maxDate:endDate});
        $( "#tDate" ).datepicker({dateFormat: "yy-mm-dd", changeYear:true, changeMonth:true, minDate:startDate, maxDate:endDate});
...
function requestImage(obj,startDate,endDate)
{
        ...
        var thisUrl="/data/make_images?begin="+ dateString(startDate) + "&end="  + dateString(endDate) + "&station=" + obj.attr("id");
    var request=$.ajax({
...
function browseImage(myRequest)
{
    ...
    if (index != -1)
        thisTimeFrame=thisTimeFrame.slice(0,index);
        var timeFrames=[];
        var myStation=$("#"+thisTimeFrame).parent();
